...
public class ArgumentParser {
	...
	public void parse(String[] args) {
		if(args==null) {
			...
			for(int i = 0 ; i < args.length ; i ++) {
				if((i+1)!=args.length && isFlag(args[i]) && isValue(args[i+1])) {
					//add a new key-value pair
					this.map.put(args[i],args[i+1]);
					i++; //move past the 
				}else if((i+1)!=args.length && isFlag(args[i]) && isFlag(args[i+1])){
					//if there is a flag with no associated value, add it to the map with null value
					this.map.put(args[i],null);
				}//else do nothing and move on
			}

			//edge case: if the last element is a flag with no associated value
			if(isFlag(args[args.length-1])) {
				this.map.put(args[args.length-1],null);
			}
		}
	}
	...
	public Path getPath(String flag) {
		String value;
		if((value = this.map.get(flag))!=null) {
	...
	/**
	 * Returns a positive integer given the flag
	 * @param flag the flag to get
	 * @param defaultValue the default value if negative
	 * @return a positive integer
	 */
	public Integer getPositiveInteger(String flag, int defaultValue) {
		String value;
		if((value = this.map.get(flag))!=null) {
